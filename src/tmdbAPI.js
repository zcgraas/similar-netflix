...
export function movieGet(){
    ...
    for(let i = 0, l = keyArr.length; i < l; i++){
        axios.get('https://api.themoviedb.org/3/discover/movie?api_key='+ {apiKey} +'&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&primary_release_date.gte=1995-01-01&primary_release_date.lte=2020-10-01&vote_count.gte=1500&with_genres=' + valArr[i])
    .then (response => {
